...
class Topic extends CI_Controller
{
    ...
    public function talk($slug, $start = 0)
    {
        ...
        if ($this->input->post('btn-edit-th')) {
            ...
            if ($this->topic_model->error_count != 0) {
                ...
                redirect('topic/talk/' . $this->topic_model->fields['th_slug'] . '/' . $start);
            }
        }
        ...
        $fposts = $this->topic_model->get_f_posts($topic->topic_id);
    ...
    public function post_delete()
    {
        ...
        $this->db->delete('tb_postlikes', array('post_id' => $post_id));
        $this->db->delete('tb_post', array('post_id' => $post_id));
        $this->session->set_userdata('tmp_success_del', 1);
    }
    //end

    //hapus post di profil
    public function post_delete_p($post_id)
    {
        // hapus
    ...
    public function savelikes()
    {
        ...
        $user_id = $this->session->userdata('bds_user_id');
        ...
        if ($resultchecklikes == '0') {
            ...
            $data = array('post_id' => $postid, 'user_id' => $user_id);
            $this->db->insert('tb_postlikes', $data);
        } else {
            $this->db->delete('tb_postlikes', array(
                'post_id' => $postid,
                'user_id' => $user_id
            ));
            $this->db->query('update tb_post set likes=likes-1 where post_id="' . $postid . '"');
    ...
    {
        $this->data['categories'] = $this->admin_model->category_get_all();
        $this->data['title']  = 'Bantuan | Forum BDS';
        $this->template->load('template', 'bantuan', $this->data);
    }
