    ...
    <p class="lead"><?php echo $user->username; ?> <span class="badge badge-secondary"><?php echo $user->level; ?></span></p>
        ...
        <?php foreach ($teman->result() as $item) { ?>               
            <?php if($this->session->userdata('bds_user_id')!=$user->user_id && $this->session->userdata('bds_logged_in')){ ?>   
                <a title="Kirim Pesan" class="btn btn-primary btn-sm" href="javascript:;" data-friend="<?= $item->user_id ?>">Kirim Pesan</a>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label>Post pertama</label>
                                <textarea name="row[f_post]" rows="70" style="height:180px;" id="post" class="form-control edit" ><?php echo $topic->f_post; ?></textarea>
